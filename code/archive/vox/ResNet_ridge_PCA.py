...
print('')
print(f'>>> Train the encoding (Ridge) model ({networks}) with PCA <<<')
print('\nInput arguments:')
...
for t_idx in tqdm(range(num_time), desc='temporal encoding'):
	...
	# =============================================================================
		ztot_eeg = tot_eeg[:, vox, t_idx].squeeze()
		# ztot_eeg = stats.zscore(tot_eeg[:, :, t_idx].squeeze(), axis=0)

		# Build the model
		# reg = LinearRegression().fit(tot_fmaps[4:], tot_eeg[:, :, t_idx])
		# reg = Ridge(alpha=100).fit(tot_fmaps[4:], ztot_eeg)
		X   = tot_fmaps[4:]
		y   = ztot_eeg
		reg   = FracRidgeRegressorCV()
		reg.fit(X, y, frac_grid=fracs)
		print(reg.best_frac_)

		# Pred eeg per voxel per time
		pred_eeg[:, vox, t_idx] = reg.predict(tot_fmaps[:4])
