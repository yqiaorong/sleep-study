def load_sleemory_full_fmaps(args):
    ...
    for f, fmaps in enumerate(tqdm(fmaps_list, desc='load sleemory images')):
        ...
        for l, dnn_layer in enumerate(all_layers):
            ...
            final_fmaps[dnn_layer] = np.squeeze(np.asarray(feats[l]))
            
    # Print result
    final_layers = final_fmaps.keys()
    for l in final_layers:
        print(f'The layer {l} has original fmaps shape (img, feat)', final_fmaps[l].shape)
