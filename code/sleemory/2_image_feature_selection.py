...
print('')
print('Feature selection of sleemory images feature maps AlexNet <<<')
print('\nInput arguments:')
...
# =============================================================================
# Feature selection model built upon THINGS images and THINGS EEG2
# =============================================================================
...
for train_subj in tqdm(range(1,11), desc='load THINGS EEG2 subjects'):
    ...
    with open(os.path.join(eeg_train_dir, 'sub-'+format(train_subj,'02'), 
        ...
        data = pickle.load(f)
        # Get the THINGS2 training channels
        if train_subj == 1:
        ...
        # Drop the stimulus channel: (16540,num_ch-1,100)
        data = np.delete(data, -1, axis=1)
        # Average the training EEG data over time: (16540,num_ch-1)
        data = np.mean(data, -1)
